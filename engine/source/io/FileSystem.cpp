...
//Different includes based on the OS that the engine is ran on
#if defined _WIN32
#include <windows.h>
...
namespace eng
{
	std::filesystem::path FileSystem::GetExecutableFolder() const //Needs different calls for each as they have OS specific functions for their paths
	{
#if defined _WIN32
	...
	std::filesystem::path FileSystem::GetAssetsFolder() const //Takes assets from root path. If root path is invalid takes it from the executable folder/assets
	{
	...
	std::vector<char> FileSystem::LoadFile(const std::filesystem::path& path)
	{
		...
		if (!file.is_open()) //File can only be searched/read if its open
		{
			return {};
		}
		...
		if (!file.read(buffer.data(), size)) //Read fails returns empty
		{
			return {};
		}
	...
	{
		auto buffer = LoadAssetFile(relativePath); //Can use to load shader source code
